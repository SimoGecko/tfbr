    ...
    class LevelPhysics : Scene {
        ...
        protected override void Build() {
            ...
            for (int i = 0; i < GameManager.numPlayers; i++) {
                ...
                GameObject forklift = new GameObject("player_" + i, Content.Load<Model>("Models/vehicles/forklift"));
            ...
            //BASE
            for (int i = 0; i < GameManager.numPlayers; i++) {
                GameObject playerBase = new GameObject("playerBase_" + i, Content.Load<Model>("Models/primitives/cube"));
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 3; ++j) {
                    GameObject body = new GameObject("domino_" + i, Content.Load<Model>("Models/primitives/cube"));
            ...
            GameObject dummy = new GameObject("dummy_object", Content.Load<Model>("Models/primitives/cube"));
        ...
        public void AddGround(GameObject parent) {
            for (int x = -2; x < 2; ++x) {
                for (int y = -2; y < 2; ++y) {
                    ...
                    GameObject groundPlane = new GameObject("groundplane", Content.Load<Model>("Models/vehicles/gplane"));
        ...
        public void AddWalls() {
            ...
            for (int x = 0; x < 40; ++x) {
                GameObject body = new GameObject("wall_" + (4 * x), Content.Load<Model>("Models/primitives/cube"));
                body.Type = ObjectType.Obstacle;
                body.transform.TranslateGlobal(new Vector3(-25 + x, y, -25));
                body.AddComponent(new RigidBodyComponent(PhysicsManager, true));


                body = new GameObject("wall_" + (4 * x + 1), Content.Load<Model>("Models/primitives/cube"));
                body.Type = ObjectType.Obstacle;
