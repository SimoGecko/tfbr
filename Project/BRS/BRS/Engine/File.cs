            ...
            using (StreamReader reader = new StreamReader(new FileStream(pathName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))) {
                ...
                while ((nameContent = reader.ReadLine()) != null) {
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        Quaternion rotation = Quaternion.CreateFromYawPitchRoll(MathHelper.ToRadians(float.Parse(rSplit[3])), MathHelper.ToRadians(float.Parse(rSplit[3])), MathHelper.ToRadians(float.Parse(rSplit[1]))); // fix your rotation
